...
if [ "${PR_STATE}" == "Closed" ]; then
    ...
    TOKEN=`curl -s -H "Content-Type: application/json" -X POST -H "Content-Type: application/json" -d '{"username": "'$docker_user'", "password":"'$docker_pass'"}' "https://hub.docker.com/v2/users/login/" | jq -r .token`
    IFS=' '
    # curl "https://hub.docker.com/v2/repositories/${TRAVIS_REPO_SLUG}/tags/${newarr[5]}/" -X DELETE -H "Authorization: JWT ${TOKEN}"
